...
`- [x]`) und faengst erst dann die naechste an. Nachschlagewerke waehrend des Bauens:
...
| `guard_context_budget` | misst den Kontextstand und warnt einmalig ab der Schwelle; mit `SCRATCHPAD_AUTO_HANDOFF=1` erzwingt er stattdessen den Handoff-Brief |
...
Den Kontext-Zyklus loest kein Command und kein Waechter von selbst aus: ist die Grenze
erreicht, **meldest du das** und der Nutzer tippt `/handoff`, dann `/clear`, dann
`/resume`. Der Waechter darf warnen und mit `SCRATCHPAD_AUTO_HANDOFF=1` das Zug-Ende
blocken, mehr nicht.
