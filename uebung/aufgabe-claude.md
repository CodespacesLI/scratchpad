...
1. Der Code landet unter `.claude/references/sources/<slug>/`.
2. `/ref` schreibt fuer jede geholte Datei den Pfad und eine Erklaerung, wofuer sie
   gut ist, in `.claude/references/INDEX.md`.
...
Claude dir sagt, dass der Zyklus dran ist. Der Warnkasten dort erklaert, warum.

## 1. Anforderungen klaeren

**Ziel:** Claude zeigt einen kurzen Brief, der die App beschreibt und deine Vorlagen nennt.
Code gibt es noch nicht.

1. Tippe `/brainstorm` und schreib dahinter deine Nachricht.

   **Deine Nachricht enthaelt:**
   - Was die App koennen soll (Liste „Was gebaut wird“).
   - Was nicht gebaut wird.
   - Dass Claude dich zu diesen drei Punkten fragen soll:
...
4. Warte, bis Claude den Brief zeigt. Pruefe:
...
5. Passt der Brief, schreib Claude, dass er so passt.

## 2. Vorlagen holen

**Ziel:** Der fertige Code fuer die Slugs aus deinem Brief liegt im Projekt, und
`.claude/references/INDEX.md` nennt ihn.

1. Tippe `/ref` und dahinter alle Slugs, die dein Brief nennt, zum Beispiel
   `/ref dnd-kit shadcn`.
2. Warte, bis Claude fertig ist. Pruefe:
   - [ ] In der Antwort steht kein `FEHLT`.
   - [ ] Die Datei `.claude/references/INDEX.md` nennt jeden Slug aus deinem Brief.

## 3. Plan erstellen

**Ziel:** Die Datei `.claude/tasks/board.md` enthaelt eine Liste kleiner Aufgaben. Die
schwierigen Aufgaben nennen eine Vorlage.

1. Tippe `/plan` und schreib dahinter deine Nachricht.

   **Deine Nachricht enthaelt:**
   - Welcher Brief gilt: der aus Schritt 1 in diesem Chat.
   - Unter welchem Namen der Plan gespeichert wird: `board.md`.
   - Dass Claude in `.claude/references/INDEX.md` nachschauen soll, wo es fertigen Code
     fuer die schwierigen Teile gibt.

2. Warte, bis Claude den Plan zeigt.
3. Oeffne die Datei `.claude/tasks/board.md`. Pruefe:
...
1. Tippe `/build` und schreib dahinter deine Nachricht.

   **Deine Nachricht enthaelt:**
   - Welcher Plan gilt: `.claude/tasks/board.md`.
   - Dass Claude alle offenen Aufgaben des Plans am Stueck baut, ohne
     zwischendurch anzuhalten.

2. Warte, bis Claude meldet, dass es fertig ist. Das dauert eine ganze Weile.
...
5. Gehen die beiden Befehle nicht, frag Claude, wie du die Tests und die App startest.
6. Oeffne die Datei `.claude/tasks/done/board.md`. Claude hat den fertigen Plan dorthin
   verschoben. Pruefe:
