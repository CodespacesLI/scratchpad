...
Arbeitsbaum prüfen**, dann beim ersten offenen Punkt weitermachen. Stand der
Vorgänger im Plan-Abbau, startet `/resume` selbst keine Arbeiter, sondern nennt den ersten
offenen Task und leitet an `/build` weiter; der neue `/build` baut ab dort alle offenen Tasks am Stück
(`<AGENTENORDNER>/vorgehen/03-build.md`, „Am Stück bis zum Ende"). So läuft nie ein
zweiter Bau neben dem ersten.
...
Briefs. Faustregel: übergeben ab etwa **120 000 Tokens**. Genau dort meldet sich auch
der Budget-Sensor. Massgeblich ist diese absolute Zahl, nicht wie voll das
technische Fenster ist: schon ab etwa 100 000 Tokens wird das Modell spürbar ungenauer,
lange bevor sein Fenster voll ist. Der Abstand bis 120 000 ist Spielraum, damit die
laufende Aufgabe noch sauber fertig wird. Das ist Context Engineering in einfacher Form.
...
Irrtum, den er für den Stand hält. Deshalb liegt die Schwelle des Sensors bei 120 000 und
nicht erst dort, wo das Fenster technisch voll ist.

**Gemeldet wird die Übergabe vom Sensor, getippt wird sie vom Menschen.** Die Sitzung
sieht ihren eigenen Füllstand nicht. In Claude Code muss der Mensch trotzdem nicht selbst
hinschauen: der Budget-Sensor misst nach jedem Zug mit und meldet sich bei etwa 120 000
Tokens. Dann **schlägt** die Sitzung den Zyklus in einem Satz vor; läuft gerade `/build`,
lässt sie vorher die laufenden Arbeiter fertig werden (siehe unten). Der Teilnehmer tippt
selbst: `/handoff`, dann `/clear`, dann
`/resume`, und lief gerade ein Bau, danach wieder `/build`. Drei Tastenfolgen plus der
erneute Bau-Aufruf, bewusst nicht wegautomatisiert. Von sich aus verspricht die Sitzung
keine Meldung bei 120k, weil sie die Zahl nicht kennt. Die Meldung kommt vom Sensor.

In Codex gibt es weder die Commands noch den Sensor. Der Mensch liest die Tokenzahl
selbst über `/status` ab und startet bei etwa 120 000 Tokens dieselben vier Schritte als
Nachrichten: eine Nachricht, die den Stand nach `<AGENTENORDNER>/state/handoff-brief.md`
schreibt und dieser Anleitung folgt; `/new` für das frische Gespräch; eine Nachricht, die
den Brief liest, gegen Dateien und `git status` prüft und den nächsten offenen Punkt
nennt; danach wieder `Baue:`.
...
**Während `/build` ist „Kontext voll" einer von genau zwei Haltegründen** (der andere ist
eine echte Blockade). Der Orchestrator lässt die laufenden Arbeiter fertig werden, prüft
und hakt ihre Tasks ab, startet keine neue Welle mehr und meldet dann den Zyklus. Vorher
hält er nicht an: nicht nach einer Welle, nicht nach einem „ersten Teil".
...
**Default: eine einzige Warnung.** Ab **120 000** Tokens meldet er sich **einmal pro
Sitzung**: der Zug endet mit der Meldung „Kontext bei ~Xk — empfiehl dem User jetzt
`/handoff`, dann `/clear` + `/resume`", und die Sitzung gibt die Empfehlung weiter. Er
erzwingt nichts und wiederholt sich nicht: eine Stempel-Datei hält die Sitzungs-Kennung
fest, alle weiteren Züge derselben Sitzung bleiben still. Der User entscheidet, ob er den Zyklus jetzt fährt
...
| Schalter | Wirkung |
|---|---|
| `SCRATCHPAD_AUTO_HANDOFF=1` | Statt einmal zu warnen, beendet der Wächter **jeden** Zug über der Schwelle mit der Auflage, den Brief **jetzt** zu schreiben; still bleibt er erst in dem Zug, in dem der Brief geschrieben wurde. Opt-in-Stufe für lange unbeaufsichtigte Läufe. |
| `SCRATCHPAD_KONTEXT_BUDGET=<zahl>` | Senkt (oder hebt) die Schwelle. Default 120000; ein unbrauchbarer Wert fällt auf den Default zurück. |
