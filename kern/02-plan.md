...
1. Vorlagen-Index lesen: `<AGENTENORDNER>/references/INDEX.md`, gleich zu Beginn. Er nennt
   pro geholter Datei den lokalen Pfad und wofür sie die Vorlage ist. So weisst du bei
   jedem schwierigen Teil (z. B. Drag-and-Drop, Dialog, Speichern) sofort, ob dafür
   fertiger Code bereitliegt. Passt eine Datei zu einem Task, wird ihr lokaler Pfad die
   Vorlage des Tasks. Gibt es `INDEX.md` nicht, bekommt kein Task eine Vorlage.
   **Kopieren schlägt neu schreiben.**
2. Projekt-Konventionen lesen: die Regel-Datei des Projekts im Wurzelverzeichnis
   (`CLAUDE.md` oder `AGENTS.md`), dazu Regeln des betroffenen Bereichs.
3. Den Code durchsuchen, statt zu raten: Welche Bausteine gibt es schon (Ansichten,
