...
def run_hook(root: str, transcript=None, raw_stdin=None, env_extra=None, **payload_extra):
    ...
    return subprocess.run([sys.executable, HOOK], input=data, capture_output=True,
                          encoding="utf-8", errors="replace", timeout=20, cwd=root,
                          env=env)
...
def test_default_schwelle_liegt_bei_120k(tmp):
    root, tr = make_project(tmp, [human_prompt(), usage_entry(115_000)])
    proc = run_hook(root, tr)
    assert proc.returncode == 0, f"115k liegt unter der Default-Schwelle: {proc.stderr}"
    root2, tr2 = make_project(os.path.join(tmp, "b"), [human_prompt(), usage_entry(125_000)])
    proc = run_hook(root2, tr2)
    assert proc.returncode == 2, f"125k liegt über der Default-Schwelle, war {proc.returncode}"
    assert "120k" in proc.stderr, proc.stderr
...
def test_kaputte_env_schwelle_faellt_auf_default(tmp):
    ...
    assert proc.returncode == 0, "kaputter ENV-Wert muss auf den Default (120k) zurückfallen"
...
def test_meldung_ist_utf8_auch_unter_fremder_codepage(tmp):
    """Umlaute kommen als gültiges UTF-8 an, auch wenn Python sonst cp1252 schreibt.

    Nachgestellt wird Windows: stderr läuft dort ohne Zutun in der Landes-Codepage,
    Claude Code liest aber UTF-8. Erzwungen per Variable, damit der Fall auf jedem
    Betriebssystem reproduzierbar ist.
    """
    root, tr = make_project(tmp, [human_prompt(), usage_entry(160_000)])
    env = dict(os.environ)
    env.pop(guard_context_budget.ENV_BUDGET, None)
    ...
    env["PYTHONUTF8"] = "0"
    payload = json.dumps({"session_id": "s1", "transcript_path": tr}).encode("utf-8")
    proc = subprocess.run([sys.executable, HOOK], input=payload, capture_output=True,
                          timeout=20, cwd=root, env=env)
    assert proc.returncode == 2, f"über der Schwelle muss warnen: {proc.stderr!r}"
    try:
        text = proc.stderr.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise AssertionError(f"stderr ist kein UTF-8: {exc}; {proc.stderr[:60]!r}")
    assert "aufgeblähter Kontext" in text, text


def test_transcript_pfad_mit_umlaut_unter_fremder_codepage(tmp):
    """Die Eingabe kommt als UTF-8: ein Umlaut im Transcript-Pfad (z.B. im Benutzernamen)
    darf unter cp1252 nicht zum falschen Pfad zerfallen und den Wächter still legen."""
    root, _ = make_project(tmp, [])
    ordner = os.path.join(tmp, "Jürgen")
    os.makedirs(ordner)
    tr = os.path.join(ordner, "transcript.jsonl")
    with open(tr, "w", encoding="utf-8") as fh:
        for entry in (human_prompt(), usage_entry(160_000)):
            fh.write(json.dumps(entry) + "\n")
    env = dict(os.environ)
    env.pop(guard_context_budget.ENV_BUDGET, None)
    env.pop(guard_context_budget.ENV_AUTO, None)
    ...
    env["PYTHONIOENCODING"] = "cp1252"
    env["PYTHONUTF8"] = "0"
    payload = json.dumps({"session_id": "s1", "transcript_path": tr},
                         ensure_ascii=False).encode("utf-8")
    proc = subprocess.run([sys.executable, HOOK], input=payload, capture_output=True,
                          timeout=20, cwd=root, env=env)
    assert proc.returncode == 2, \
        f"Transcript unter Umlaut-Pfad muss gemessen werden, war {proc.returncode}"


TESTS = [
    test_unter_schwelle_bleibt_still,
    test_default_schwelle_liegt_bei_120k,
    ...
    test_transcript_pfad_mit_umlaut_unter_fremder_codepage,
]
